...
SEG_ROW_RATIO = 0.6       # 行投影过滤阈值 = ratio * 图像宽度
...
# 上边缘：只用粗 y_top 附近的窗口
# 下边缘：两个窗口——粗轮廓窗口 + 厚度先验窗口
HALF_WINDOW_TOP = 20         # 上边缘搜索半窗（像素）
HALF_WINDOW_BOT_CNT = 20    # 以粗 y_bot 为中心的半窗
HALF_WINDOW_BOT_PRI = 10     # 以厚度先验 y_top+T_prior 为中心的半窗
# 厚度先验（软约束）
...
def refine_edges_with_local_gradient(
    ...
    """
    利用局部灰度梯度，对上下边缘做一次细化（软先验版）：
      - 上边缘：在粗 y_top 附近，只考虑暗->亮的正梯度；
      - 下边缘：搜索区 = 粗 y_bot 窗口 ∩ 厚度先验窗口，
        在该区内只考虑亮->暗负梯度，并用先验做软约束打分；
      - 梯度必须通过“绝对 + 相对”阈值，再按 score=grad-penalty 选点。
    """
    h, w = roi_gray.shape[:2]

    # 归一化成 0~1 的 float，方便看梯度
    ...
    if use_thickness_prior:
        ...
        print(f"使用厚度先验(软约束): {thickness_prior_pix:.3f} 像素")
    else:
        thickness_prior_pix = None

    # 方便查找每一列的上/下粗边缘位置
    top_map = {int(x): float(y) for x, y in zip(x_top.astype(int), y_top_ref)}
    bot_map = {int(x): float(y) for x, y in zip(x_bot.astype(int), y_bot_ref)}

    # ---------- 上边缘：只用方向梯度，必要时退回 abs(diff) ----------
    def refine_top_one(col, y0_int):
        y_center = int(round(y0_int))
        y0 = max(1, y_center - half_window_top)
        y1 = min(h - 2, y_center + half_window_top)
        if y1 <= y0 + 2:
        ...
        grad = diff.copy()          # 暗->亮：正梯度
        grad[grad < 0.0] = 0.0
        if np.all(grad <= 0.0):
            # 如果方向信息完全没用，就退回到 abs(diff)
            grad = np.abs(diff)
    ...
    # ---------- 下边缘：轮廓窗口 ∩ 先验窗口 + 阈值 + 打分 ----------
    def refine_bottom_one(col, x_i, y0_int):
        # 粗轮廓中心
        yc_cnt = float(bot_map.get(int(x_i), float(y0_int)))

        # 厚度先验中心
        if use_thickness_prior and int(x_i) in top_map:
            yc_pri = float(top_map[int(x_i)] + thickness_prior_pix)
        else:
            yc_pri = yc_cnt

        # 轮廓窗口
        low1 = yc_cnt - half_window_cnt
        high1 = yc_cnt + half_window_cnt
        # 先验窗口
        low2 = yc_pri - half_window_pri
        high2 = yc_pri + half_window_pri
        ...
        y0_win = max(1, int(round(low)))
        y1_win = min(h - 2, int(round(high)))
        if y1_win <= y0_win + 2:
            return float(yc_cnt), False

        profile = col[y0_win:y1_win + 1]
        diff = np.diff(profile)
        ...
        if g_max <= 0.0:
            # 没有明显的亮->暗梯度，这一列视为低置信度
            return float(yc_cnt), False

        # 绝对 + 相对 阈值
        thr = max(grad_abs_thr, grad_rel_thr * g_max)
        idx = np.where(grad >= thr)[0]
        if idx.size == 0:
            # 实在没有通过阈值的，就退而求其次拿最大值
            idx = np.array([int(np.argmax(grad))])

        # 先验惩罚系数：越偏离 yc_pri，score 越小
        lam = prior_lambda * g_max
        ...
        for k in idx:
            ...
            y_pos = y0_win + k
            if use_thickness_prior:
                penalty = lam * abs(y_pos - yc_pri)
            else:
                penalty = 0.0
            score = float(grad[k]) - penalty
            if score > best_score:
                best_score = score
                best_k = k
        ...
        y_peak = y0_win + k + offset
        return float(y_peak), True
    ...
    for i, (x_i, y_i) in enumerate(zip(x_top.astype(int), y_top.astype(int))):
        if 0 <= x_i < w:
            col = roi_f[:, x_i]
            y_top_ref[i] = refine_top_one(col, y_i)

    # 下边缘
    for i, (x_i, y_i) in enumerate(zip(x_bot.astype(int), y_bot.astype(int))):
        if 0 <= x_i < w:
            col = roi_f[:, x_i]
            y_ref, ok = refine_bottom_one(col, x_i, y_i)
            # 这里即使 ok=False，我们也先用 y_ref（通常是粗轮廓中心）填上，
            # 后面拟合时再通过厚度 outlier 筛掉。
            y_bot_ref[i] = y_ref
...
def main():
    roi_gray = cv2.imread(ROI_IMAGE_PATH, cv2.IMREAD_GRAYSCALE)
    if roi_gray is None:
        print("读图失败")
        return
    ...
    y_top_ref, y_bot_ref = refine_edges_with_local_gradient(
        ...
        x_top, y_top,
        x_bot, y_bot,
        half_window_top=HALF_WINDOW_TOP,
        half_window_cnt=HALF_WINDOW_BOT_CNT,
        half_window_pri=HALF_WINDOW_BOT_PRI,
        use_thickness_prior=USE_THICKNESS_PRIOR,
        thickness_prior_pix=THICKNESS_PRIOR_PIX,
        grad_abs_thr=GRAD_ABS_THR,
        grad_rel_thr=GRAD_REL_THR,
