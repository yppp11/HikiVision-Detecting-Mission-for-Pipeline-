...
def refine_edges_with_local_gradient(roi_gray,
                                     x_top, y_top,
                                     x_bot, y_bot,
                                     half_window=4):
    """
    ...
    # 一个小工具：给定列 x 和大致 y，在局部窗口里找梯度峰值，并做一个简单的二次插值
    def refine_one(col, y0_int):
        y0 = max(1, y0_int - half_window)
        y1 = min(h - 2, y0_int + half_window)
        if y1 <= y0 + 2:
            return float(y0_int)

        profile = col[y0:y1 + 1]              # 灰度
        grad = np.abs(np.diff(profile))       # 一阶差分当梯度

        k = int(np.argmax(grad))              # 梯度最大处（在 grad 里的下标）
        # 对 grad[k-1], grad[k], grad[k+1] 做抛物线拟合，估计亚像素偏移
        offset = 0.0
        if 0 < k < len(grad) - 1:
            g0, g1, g2 = grad[k - 1], grad[k], grad[k + 1]
            denom = (g0 - 2 * g1 + g2)
            if abs(denom) > 1e-6:
                offset = 0.5 * (g0 - g2) / denom   # 经典三点抛物插值

        y_peak = y0 + k + offset
        return float(y_peak)

    # 上边缘细化
    for i, (x, y) in enumerate(zip(x_top.astype(int), y_top.astype(int))):
        if 0 <= x < w:
            col = roi_f[:, x]
            y_top_ref[i] = refine_one(col, y)

    # 下边缘细化
    for i, (x, y) in enumerate(zip(x_bot.astype(int), y_bot.astype(int))):
        if 0 <= x < w:
            col = roi_f[:, x]
            y_bot_ref[i] = refine_one(col, y)
