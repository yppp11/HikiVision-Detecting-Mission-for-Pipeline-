...
# 列投影 + 平滑
COL_MIN_WHITE_RATIO = 0.2   # 每列白点（前景）数量至少占整幅高度的比例，过小认为是噪声列
...
# 分块 Otsu 相关参数（自适应阈值）
USE_BLOCK_OTSU = True          # 是否启用分块 Otsu
BLOCK_OTSU_SIZE = (2, 1500)     # 分块大小 (宽, 高)，可根据图像大小调
BLOCK_OTSU_TOP_BIAS = -3       # 上边缘 Otsu 阈值微调偏置（灰度值）
BLOCK_OTSU_BOT_BIAS = +3       # 下边缘 Otsu 阈值微调偏置（灰度值）
...
# 工具函数：自适应ostu
def block_otsu_threshold(img, block_size, thr_bias):
    """
    对 img 按 block_size 分块做 Otsu，自定义偏置 thr_bias，
    最后拼成一整幅二值图。

    img : 8bit 单通道图像（已经高斯模糊过）
    block_size : (bw, bh)
    thr_bias : 在 Otsu 阈值基础上加减一点，调节前景范围
    """
    h, w = img.shape[:2]
    bw, bh = block_size
    ...
    for y0 in range(0, h, bh):
        for x0 in range(0, w, bw):
            ...
            # 阈值微调
            thr = np.clip(thr + thr_bias, 0, 255)

            # 再用调过的阈值做一次普通二值化
            _, tile_bin = cv2.threshold(
                tile, thr, 255, cv2.THRESH_BINARY
            )

            binary[y0:y1, x0:x1] = tile_bin
...
def segment_and_get_mask(
    ...
    if use_block_otsu:
        # 分块 Otsu 自适应阈值
        binary = block_otsu_threshold(
            blur,
            block_size=block_size,
            thr_bias=thr_bias
        )
        print(f"分块 Otsu: block={block_size}, bias={thr_bias}")
    else:
        # 老的固定阈值分割逻辑，兼容保留
        _, binary = cv2.threshold(blur, thr_val, 255, cv2.THRESH_BINARY)
        print(f"使用固定阈值: {thr_val}")
    # ==============================================
...
def main():
    ...
    binary_top = segment_and_get_mask(
        ...
        norm_min=NORM_MIN,
        norm_max=NORM_MAX,
        use_block_otsu=USE_BLOCK_OTSU,
        block_size=BLOCK_OTSU_SIZE,
        thr_bias=BLOCK_OTSU_TOP_BIAS,
    )

    # ---- 2) 下边缘：分块 Otsu + 轻微升高阈值 ----
