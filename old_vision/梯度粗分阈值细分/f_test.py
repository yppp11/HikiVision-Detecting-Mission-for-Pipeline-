...
def find_edges_gradient_coarse(img):
    ...
    for x in range(w):
        ...
        for y in range(2, lim_t):
            if col_grads[y] > th_t:
                # 也可以加上 Look-Ahead，但一般上边缘光照好，不太需要
                x_top.append(x)
                y_top.append(y)
                break
        
        # === 下边缘 (增加 Look-Ahead 抗虚影) ===
        start_b = int(h * (1 - SEARCH_LIMIT_RATIO_BOT))
        reg_b = col_grads[start_b:h]
        # 注意：reg_b 里是负数，min 是绝对值最大的负数
        min_val = np.min(reg_b) if reg_b.size else 0
        
        # 阈值计算
        th_bot = min(-MIN_ABS_THRESHOLD, min_val * RELATIVE_THRESH_BOT)
        
        # 从底向上扫
        for y in range(h - 3, start_b, -1):
            # 1. 触发了初步阈值 (可能是虚影，也可能是真边缘)
            if col_grads[y] < th_bot:
                
                # 2. 【核心修改】开启"上帝视角"往里多看几步
                # 我们要检查范围 [y - LOOK_AHEAD_WINDOW, y] 这一小段
                # 注意：y 是从大到小变化的，所以"往里"意味着索引减小
                peek_start = max(0, y - LOOK_AHEAD_WINDOW)
                peek_end = y + 1 # 切片右边界
                
                local_window = col_grads[peek_start : peek_end]
                
                if local_window.size > 0:
                    # 找到局部窗口里"最负"（梯度最强）的那个点的位置
                    # argmin 返回的是相对窗口的索引
                    best_local_idx = np.argmin(local_window)
...
def main():
    ...
    # yb = apply_savgol_filter(yb, window_length=61, polyorder=2)
    # Step 4: 拟合
